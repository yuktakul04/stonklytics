...
export default function Watchlist({ isOpen, onClose, onStockSelect }) {
    ...
    // Refetch watchlist when sidebar opens
    useEffect(() => {
        if (isOpen && user) {
            fetchWatchlist()
        }
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-6 border-b border-dark-border bg-dark-surface">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-white">My Watchlists</h2>
                            <button
                                onClick={onClose}
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        {/* Error Message */}
                        {error && (
                            <div className="mb-4 bg-red-950/20 border border-red-500/50 text-red-300 px-4 py-3 rounded-lg">
                        ...
                        {watchlists.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <h3 className="text-lg font-semibold text-white mb-2">No watchlists yet</h3>
                                <p className="text-sm text-gray-300 mb-4">Create your first watchlist to get started</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {watchlists.map((watchlist) => (
                                    <div key={watchlist.id} className="bg-dark-surface border border-dark-border rounded-lg p-4 hover:bg-dark-hover transition-colors duration-200">
                                        ...
                                        {watchlist.items && watchlist.items.length > 0 && (
                                            <div className="space-y-2 mt-3">
                                                {watchlist.items.map((item, idx) => {
                                                    ...
                                                    return (
                                                        <div key={idx} className="flex items-center justify-between p-3 bg-dark-card rounded-lg border border-dark-border hover:border-gray-500 hover:bg-dark-hover transition-all duration-200">
                                                            <button
                                                                onClick={() => onStockSelect(ticker)}
                                                                className="flex-1 text-left transition-colors"
                                                            >
                                                                <div className="flex items-center gap-2">
                                                                    <span className="font-semibold text-white text-base">{ticker}</span>
                                                                    {item.name && (
                                                            ...
                                                            <button
                                                                ...
                                                                className="text-gray-400 hover:text-red-400 transition-colors p-1.5 ml-2 rounded hover:bg-red-950/20"
                                                                title="Remove stock"
                                                            >
                    ...
                    <div className="p-6 border-t border-dark-border bg-dark-surface">
                        {!showCreateForm ? (
                            <button
                                ...
                                className="w-full px-4 py-2.5 bg-white text-dark-bg hover:bg-gray-200 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2"
                            >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <form onSubmit={handleCreateSubmit} className="space-y-2">
                                <input
                                    ...
                                    className="w-full px-3 py-2 bg-dark-surface border border-dark-border rounded-lg text-white placeholder-gray-400 focus:ring-2 focus:ring-gray-500 focus:border-gray-600 outline-none"
                                    autoFocus
                                />
                                <div className="flex space-x-2">
                                    <button
                                        type="submit"
                                        disabled={creating}
                                        className="flex-1 px-4 py-2 bg-white text-dark-bg hover:bg-gray-200 rounded-lg font-medium transition-all duration-200 disabled:bg-gray-800 disabled:text-gray-500"
                                    >
